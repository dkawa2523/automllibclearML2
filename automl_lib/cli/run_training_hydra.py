...
from automl_lib.workflow import run_training


@hydra.main(version_base=None, config_path=str(conf_root()), config_name="app/training")
def main(cfg: DictConfig) -> None:
    cfg_dict = to_clean_dict(cfg)
    validated = TrainingConfig.model_validate(cfg_dict)
    if maybe_clone_from_config(validated, phase="training", output_info=None):
        return
    clearml_avoid_task_reuse()

    cfg_path = write_yaml_config(cfg_dict, prefix="training")
