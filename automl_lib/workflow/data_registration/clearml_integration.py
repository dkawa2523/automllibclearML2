"""
ClearML integration for data_registration phase.
タスク生成を扱う。
"""
...
def create_data_registration_task(config: Dict[str, Any], parent_task_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    tags = build_tags(ctx, phase="data_registration", extra=clearml_cfg.get("tags"))
    mgr = ClearMLManager(
        clearml_settings,
        task_name=task_name("data_registration", ctx),
    ...
    try:
        mgr.connect_params_sections(
            {
                "Input": {"csv_path": (config.get("data") or {}).get("csv_path") or ""},
            }
        )
    except Exception:
        pass
...
def finalize_data_registration_task(task, artifact_paths=None) -> None:
